...
const initialModules = [
  { id: 'vizora', name: 'Vizora', description: 'AI-powered photo, video and ad creation studio.', status: 'active', type: 'AI Module', icon: '✦', href: '/vizora', customerLink: 'https://scalevyapar.vercel.app/vizora', features: ['AI Photo Generation', 'Photo Upscaling 4x', 'Video Ads', 'UGC Creator Videos', 'Magic Eraser'], color: '#7c3aed' },
  { id: 'crm', name: 'CRM Module', description: 'Customer relationship management and call tracking.', status: 'coming_soon', type: 'Standard', icon: '◈', href: '#', customerLink: '', features: ['Contact Management', 'Call Tracking', 'Follow-ups', 'Pipeline'], color: '#0284c7' },
  { id: 'inventory', name: 'Inventory Module', description: 'Stock management and production tracking.', status: 'coming_soon', type: 'Standard', icon: '◉', href: '#', customerLink: '', features: ['Stock Tracking', 'Production Orders', 'Raw Materials', 'Dispatch'], color: '#059669' },
  { id: 'whatsapp', name: 'WhatsApp Module', description: 'Bulk messaging, chatbot and lead automation.', status: 'coming_soon', type: 'Standard', icon: '◎', href: '#', customerLink: '', features: ['Bulk Messaging', 'Chatbot', 'Auto Replies', 'Lead Capture'], color: '#16a34a' },
  { id: 'leads', name: 'Lead Generation', description: 'Google data extractor for B2B leads.', status: 'coming_soon', type: 'Standard', icon: '◎', href: '#', customerLink: '', features: ['Google Maps Scraper', 'Filter by Location', 'Filter by Business', 'Export CSV'], color: '#d97706' },
  { id: 'shopify', name: 'Shopify / Website', description: 'Website setup, product catalog and order management.', status: 'coming_soon', type: 'Standard', icon: '◇', href: '#', customerLink: '', features: ['Shopify Setup', 'Product Catalog', 'Order Management', 'Pricing'], color: '#84cc16' },
]

const initialClients = [
  { id: 'client-1776346123958-g7l955z69', name: 'Neelufer Creations', email: 'neelufercreation@gmail.com', phone: '+91 9876543210', plan: 'Growth', status: 'active', assignedModules: ['vizora'], joinedAt: '2026-04-16' },
]
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        {data.features.map((f: string, i: number) => (
          <div key={i} style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
            ...
            {data.features.length > 1 && <button onClick={() => setData({ ...data, features: data.features.filter((_: any, idx: number) => idx !== i) })} style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: '6px', padding: '0 10px', cursor: 'pointer', fontSize: '16px' }}>×</button>}
          </div>
        ))}
              ...
              {modules.map(mod => (
                <div key={mod.id}>
                  {editingModuleId === mod.id && editModule ? (
                    ...
                    <div style={{ background: 'white', border: `1px solid ${mod.status === 'active' ? '#a78bfa50' : '#e2e8f0'}`, borderRadius: '18px', overflow: 'hidden', boxShadow: mod.status === 'active' ? '0 4px 20px rgba(124,58,237,0.08)' : 'none' }}>
                      <div style={{ height: '5px', background: mod.color }} />
                      <div style={{ padding: '20px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                          <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: mod.color + '15', border: `1px solid ${mod.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px' }}>{mod.icon}</div>
                          <span style={{ background: mod.status === 'active' ? '#f0fdf4' : '#f8fafc', color: mod.status === 'active' ? '#16a34a' : '#94a3b8', fontSize: '11px', padding: '3px 10px', borderRadius: '99px', fontWeight: '700', border: `1px solid ${mod.status === 'active' ? '#bbf7d0' : '#e2e8f0'}` }}>
                        ...
                        {mod.customerLink && (
                          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '8px', padding: '8px 10px', marginBottom: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <button onClick={() => copyLink(mod.customerLink, mod.id)} style={{ background: '#16a34a', color: 'white', border: 'none', fontSize: '10px', padding: '3px 8px', borderRadius: '4px', cursor: 'pointer', fontWeight: '600', whiteSpace: 'nowrap' }}>{copied === mod.id ? '✓' : 'Copy'}</button>
                          </div>
                        )}
          ...
          <div>
                    ...
                    {modules.map(mod => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: isSelected ? mod.color : 'transparent', border: `2px solid ${isSelected ? mod.color : '#d1d5db'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '11px', fontWeight: '700', flexShrink: 0 }}>
                            {isSelected ? '✓' : ''}
                          </div>
                        </div>
                      )
            ...
            {clients.length === 0 && !addingClient ? (
              <div style={{ background: 'white', border: '2px dashed #e2e8f0', borderRadius: '16px', padding: '48px', textAlign: 'center' }}>
                ...
                <p style={{ color: '#64748b', fontSize: '16px', fontWeight: '600', margin: '0 0 8px' }}>No clients yet</p>
                <p style={{ color: '#94a3b8', fontSize: '13px', margin: '0 0 20px' }}>Add your first client to start assigning modules</p>
                <button onClick={() => setAddingClient(true)} style={{ background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', color: 'white', border: 'none', fontSize: '13px', fontWeight: '700', padding: '10px 24px', borderRadius: '10px', cursor: 'pointer' }}>+ Add First Client</button>
              </div>
            ) : (
                ...
                {clients.map(client => (
                  <div key={client.id} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '16px', overflow: 'hidden' }}>
                    <div style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <span style={{ background: '#ede9fe', color: '#7c3aed', fontSize: '12px', padding: '4px 12px', borderRadius: '99px', fontWeight: '600' }}>{client.plan.split(' —')[0]}</span>
                        ...
                        <button onClick={() => setEditingClient(editingClient === client.id ? null : client.id)} style={{ background: editingClient === client.id ? '#ede9fe' : '#f8fafc', color: editingClient === client.id ? '#7c3aed' : '#64748b', border: `1px solid ${editingClient === client.id ? '#7c3aed' : '#e2e8f0'}`, fontSize: '12px', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>
                          {editingClient === client.id ? 'Done ✓' : 'Edit Modules'}
                        </button>
                        {deleteClientConfirm === client.id ? (
                          <div style={{ display: 'flex', gap: '4px' }}>
                            <button onClick={() => deleteClient(client.id)} style={{ background: '#dc2626', color: 'white', border: 'none', fontSize: '11px', padding: '6px 10px', borderRadius: '6px', cursor: 'pointer', fontWeight: '700' }}>Delete!</button>
                            <button onClick={() => setDeleteClientConfirm(null)} style={{ background: '#f1f5f9', color: '#64748b', border: '1px solid #e2e8f0', fontSize: '11px', padding: '6px 10px', borderRadius: '6px', cursor: 'pointer' }}>No</button>
                          </div>
                        ) : (
                          <button onClick={() => setDeleteClientConfirm(client.id)} style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', fontSize: '12px', padding: '6px 10px', borderRadius: '8px', cursor: 'pointer' }}>🗑</button>
                        )}
                      </div>
                    </div>
