...
const getWorkerFileContext = async (workerId: string, documentKind: string) => {
  ...
  return {
    documentKind,
    field,
    storagePath: worker[field].trim()
  }
}

const resolveUploadContentType = (file: File, documentKind: WorkerDocumentKind) => {
...
const removeStoragePath = async (storagePath: string) =>
  supabaseAdmin.storage.from(WORKER_UPLOAD_BUCKET).remove([storagePath])
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const contentType = resolveUploadContentType(file, context.documentKind)
    if (!contentType) {
    ...
    if (context.storagePath && context.storagePath !== uploadedPath) {
      const { error: removeError } = await removeStoragePath(context.storagePath)
      if (removeError) {
    ...
    if (uploadedPath && !keepUploadedFile) {
      await removeStoragePath(uploadedPath).catch(() => undefined)
    }
