...
export async function POST(request: NextRequest) {
  try {
    ...
    const reusableDeletedCompanyLogin = existingUser?.role === 'CLIENT'

    if (existingUser && !reusableDeletedCompanyLogin) {
    ...
    if (reusableDeletedCompanyLogin && existingUser) {
      await updateClient(existingUser.id, {
        name: companyName,
        email,
        phone: mobile,
