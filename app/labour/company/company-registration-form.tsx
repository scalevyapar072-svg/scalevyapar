...
export function CompanyRegistrationForm({
  ...
  const visibleStateOptions = useMemo(
    () => buildLabourMasterSelectOptions(getVisibleLabourMasterOptions(stateOptions)),
    [stateOptions]
  )
  ...
  const validate = () => {
    ...
    if (!form.industryCategory) nextErrors.industryCategory = 'Please select industry category.'
    if (!form.businessType) nextErrors.businessType = 'Please select business type.'
  ...
  const uploadDocument = async (key: UploadKey, documentKind: string, fileOverride?: File | null) => {
    const current = uploads[key]
    const selectedFile = fileOverride || current.file
    if (!selectedFile) {
    ...
    try {
      ...
      const fileName = String(data.fileName || selectedFile.name)
  ...
  const handleFileChange = (key: UploadKey, file: File | null) => {
    ...
    void uploadDocument(key, UPLOAD_DOCUMENT_KINDS[key], file).catch(() => undefined)
  }
  ...
  const uploadDocumentIfPresent = async (key: UploadKey, documentKind: string) => {
    ...
    if (current.status === 'uploaded' && current.storagePath) {
      return {
        fileName: current.fileName || current.file.name,
    ...
    if (current.status === 'uploading') {
      throw new Error('Please wait for the document upload to finish before submitting.')
    }

    return uploadDocument(key, documentKind, current.file)
  }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const gstCertificate = await uploadDocumentIfPresent('gstCertificate', 'gst_certificate')
            ...
            {state.status === 'uploaded'
              ...
              : state.status === 'uploading'
                ...
                : state.status === 'error'
                  ...
                  : state.status === 'ready'
                    ? 'Selected. Upload starts automatically.'
                    : 'Select a JPG, PNG, WEBP, or PDF file to continue'}
          </p>
        ...
        <div className={styles.companyRegisterFormWrap}>
          ...
          {success ? (
            <div className={styles.companyRegisterSuccessCard}>
              ...
              <div className={styles.companyRegisterSuccessActions}>
                <Link href={resolveHref('/labour/company/panel')} className={styles.companyRegisterPrimaryButton}>Open Company Panel</Link>
                <Link href={resolveHref('/labour/company/signin')} className={styles.companyRegisterSecondaryButton}>Login Dashboard</Link>
              </div>
            </div>
          ) : null}
          ...
          <form className={styles.companyRegisterForm} onSubmit={handleSubmit} noValidate>
            <div className={styles.companyRegisterSection}>
              ...
              <div className={styles.companyRegisterGridTwo}>
                ...
                <div>
                  ...
                  >
                    <option value="">{form.industryCategory ? 'Select business type' : 'Select industry category first'}</option>
                    {visibleBusinessTypeOptions.map(item => <option key={item.id} value={item.value}>{item.label}</option>)}
                  </select>
                  {!form.industryCategory && !errors.businessType ? (
                    <p className={styles.companyRegisterSectionNote}>Select industry category first</p>
                  ) : null}
              ...
              <div className={styles.companyRegisterAddressSection}>
                ...
                <div className={styles.companyRegisterLocationGrid}>
                  <div>
                    ...
                    <select
                      ...
                      onChange={event => {
                        ...
                        setErrors(current => ({ ...current, state: '', city: '', submit: '' }))
                      }}
                    >
                      <option value="">Select state</option>
                      {visibleStateOptions.map(state => <option key={state.id} value={state.value}>{state.label}</option>)}
                    </select>
                    {visibleStateOptions.length === 0 && !errors.state ? (
                      <p className={styles.companyRegisterSectionNote}>State options are currently unavailable.</p>
                    ) : null}
            ...
            <div className={styles.companyRegisterSection}>
              ...
              <p className={styles.companyRegisterSectionNote}>Optional. Files upload immediately after selection so you can confirm success before submitting.</p>
