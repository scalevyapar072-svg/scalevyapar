...
export default async function HomePage() {
  ...
  const tools = content.home.toolsSection.tools
  const steps = content.home.stepsSection.steps
  const whyCards = content.home.whySection.cards
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <AnimateOnScroll direction="up">
            <div style={{ textAlign: 'center', marginBottom: '56px' }}>
              ...
              <p className="section-subtitle">Real businesses, real results. Here's what our clients have to say about ScaleVyapar.</p>
            </div>
          </AnimateOnScroll>
  ...
  .hero-mobile-wrap { display: flex !important; }
        ...
        <div className="hero-desktop-wrap">
          ...
          <div className="hero-text-left">
            <span className="hero-badge">{content.home.hero.badge}</span>
            <h1 className="hero-title">
              {content.home.hero.titlePrefix}<br />with <TypingAnimation words={content.home.hero.titleWords} />
            </h1>
            <p className="hero-subtitle">
              {content.home.hero.subtitle}
            </p>
            <div className="hero-btns">
              <Link href={content.home.hero.primaryCtaHref} className="hero-btn-main">{content.home.hero.primaryCtaLabel}</Link>
              <Link href={content.home.hero.secondaryCtaHref} className="hero-btn-sec">{content.home.hero.secondaryCtaLabel}</Link>
            </div>
            <div className="hero-stats">
              {content.home.hero.stats.map(stat => (
          ...
          <div className="hero-scroll-hint">
            ...
            <span>{content.home.hero.scrollHint}</span>
          </div>
        </div>
        ...
        <div className="hero-mobile-wrap">
          ...
          <div className="hero-text-top">
            ...
            <p className="hero-subtitle">
              {content.home.hero.mobileSubtitle}
            </p>
            <div className="hero-btns">
              <Link href={content.home.hero.primaryCtaHref} className="hero-btn-main">{content.home.hero.mobilePrimaryCtaLabel}</Link>
              <Link href={content.home.hero.secondaryCtaHref} className="hero-btn-sec">{content.home.hero.secondaryCtaLabel}</Link>
            </div>
            <div className="hero-stats">
              {content.home.hero.stats.map(stat => (
                <div key={stat.mobileLabel || stat.label} className="hero-stat">
                  <h4>{stat.value}</h4>
                  <p>{stat.mobileLabel || stat.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <CounterBanner items={content.home.counterBanner.items} />
      {/* Tools */}
      <section className="tools-section">
        <div className="section-header">
          <AnimateOnScroll direction="up">
            <span className="section-tag">{content.home.toolsSection.eyebrow}</span>
            <h2 className="section-title">{content.home.toolsSection.title}</h2>
            <p className="section-subtitle">{content.home.toolsSection.subtitle}</p>
          </AnimateOnScroll>
        </div>
          ...
          {tools.map((tool, idx) => (
            <AnimateOnScroll key={tool.name} direction="up" delay={idx * 100}>
              <div className="tool-card">
                ...
                <h3>{tool.name}</h3>
                <span className={`tool-tag ${tool.tag === 'Built by Us' ? 'own' : 'premium'}`}>★</span>
     ...
     <ToolPreview content={content.home.previewSection} />
      {/* How It Works */}
      <section className="how-section">
        <div className="section-header">
          <AnimateOnScroll direction="up">
            <span className="section-tag">{content.home.stepsSection.eyebrow}</span>
            <h2 className="section-title">{content.home.stepsSection.title}</h2>
            <p className="section-subtitle">{content.home.stepsSection.subtitle}</p>
          </AnimateOnScroll>
        </div>
      ...
      <section className="why-section">
        <div className="section-header">
          <AnimateOnScroll direction="up">
            <span style={{ background: 'rgba(255,255,255,0.15)', color: 'white', padding: '6px 16px', borderRadius: '20px', fontSize: '13px', fontWeight: '600', display: 'inline-block', marginBottom: '16px' }}>{content.home.whySection.eyebrow}</span>
            <h2 className="section-title" style={{ color: 'white' }}>{content.home.whySection.title}</h2>
            <p className="section-subtitle" style={{ color: 'rgba(255,255,255,0.7)' }}>{content.home.whySection.subtitle}</p>
          </AnimateOnScroll>
        </div>
      ...
      <TestimonialsSection content={content.home.testimonialsSection} />
      <ComparisonTable content={content.home.comparisonSection} />
      {/* CTA */}
        ...
        <AnimateOnScroll direction="up">
          <div className="cta-box">
            ...
            <div className="cta-btns">
              <Link href={content.home.finalCta.primaryCtaHref} className="cta-btn-white">{content.home.finalCta.primaryCtaLabel}</Link>
              <a href={content.home.finalCta.secondaryCtaHref} className="cta-btn-wa" target="_blank" rel="noreferrer">{content.home.finalCta.secondaryCtaLabel}</a>
            </div>
          </div>
        </AnimateOnScroll>
      </section>
