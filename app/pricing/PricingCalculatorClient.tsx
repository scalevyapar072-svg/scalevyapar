...
export default function PricingCalculator({
  ...
  const selectedTools = content.tools.filter(tool => selected.includes(tool.name))
  const monthlyTotal = selectedTools.reduce((sum, tool) => sum + tool.monthly, 0)
  const oneTimeTotal = selectedTools
    .filter((tool): tool is typeof tool & { oneTime: number } => typeof tool.oneTime === 'number')
    .reduce((sum, tool) => sum + tool.oneTime, 0)
  const totalCredits = selectedTools.length * 500

  const whatsappMessage = selectedTools.length > 0
    ? `Hi! I want to subscribe to ScaleVyapar with these tools: ${selectedTools.map(tool => tool.name).join(', ')}. Monthly total: ₹${monthlyTotal.toLocaleString()}${oneTimeTotal > 0 ? `. One-time setup: ₹${oneTimeTotal.toLocaleString()}` : ''}. Monthly credits: ${totalCredits}.`
    ...
    <>
      ...
      <div className="tab-bar">
        <button className={`tab-btn ${activeTab === 'plan' ? 'active' : ''}`} onClick={() => setActiveTab('plan')}>
          {content.planTabLabel}
        </button>
        <button className={`tab-btn ${activeTab === 'credits' ? 'active' : ''}`} onClick={() => setActiveTab('credits')}>
          {content.creditsTabLabel}
        </button>
      </div>

      {activeTab === 'plan' && (
        <div>
          <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '20px', textAlign: 'center' }}>{content.planIntro}</p>

          {content.tools.map(tool => (
            <div key={tool.name} className={`tool-card ${selected.includes(tool.name) ? 'selected' : ''}`} onClick={() => toggle(tool.name)}>
              <div className="tool-header">
                <div className="tool-left">
                  <div className={`tool-icon-box ${selected.includes(tool.name) ? 'selected' : ''}`}>{tool.icon}</div>
                  <div>
                ...
                <div className="tool-right">
                  <div className="tool-price">
                    ...
                    {typeof tool.oneTime === 'number' ? <span className="one-time-badge">+₹{tool.oneTime.toLocaleString()} setup</span> : null}
                  </div>
                  <input type="checkbox" className="tool-checkbox" checked={selected.includes(tool.name)} onChange={() => toggle(tool.name)} onClick={event => event.stopPropagation()} />
                </div>
              </div>
              ...
              <>
                ...
                {oneTimeTotal > 0 ? (
                  <div className="summary-row">
                    <span>{content.websiteSetupLabel}</span>
                    <span>₹{oneTimeTotal.toLocaleString()}</span>
                  </div>
                ) : null}
                <div className="summary-total-row">
                  <span>{content.monthlyTotalLabel}</span>
                  <span>₹{monthlyTotal.toLocaleString()}/mo</span>
                </div>
                <div className="summary-credits">
                  <p>{content.monthlyCreditsLabel}</p>
                  <h4>⚡ {totalCredits.toLocaleString()} credits/month</h4>
                </div>
              </>
            )}
            <a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappMessage)}`} className="wa-btn" target="_blank" rel="noreferrer">
      ...
      {activeTab === 'credits' && (
        <div>
          <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '24px', textAlign: 'center' }}>{content.creditsIntro}</p>
          {content.creditPacks.map(pack => (
            <div key={pack.credits} className={`credit-pack ${pack.popular ? 'popular' : ''}`}>
              {pack.popular ? <span className="popular-badge">⭐ Most Popular</span> : null}
              ...
              <div className="credit-pack-right">
                ...
                <a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hi! I want to buy ${pack.credits} extra credits (${pack.label}) for ₹${pack.price}.`)}`} className="credit-wa-btn" target="_blank" rel="noreferrer">
                  Buy Now →
                </a>
              </div>
            </div>
          ...
          <div style={{ background: '#f8fafc', borderRadius: '16px', padding: '24px', marginTop: '24px', textAlign: 'center', border: '1px solid #e2e8f0' }}>
            <p style={{ fontSize: '24px', marginBottom: '12px' }}>{content.customPackageTitle}</p>
            <h3 style={{ color: '#1e293b', fontSize: '16px', fontWeight: '700', marginBottom: '8px' }}>{content.customPackageTitle}</h3>
            <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '16px' }}>{content.customPackageText}</p>
            <a href={`https://wa.me/${whatsappNumber}`} style={{ background: '#374655', color: 'white', padding: '12px 24px', borderRadius: '10px', fontWeight: '700', fontSize: '14px', display: 'inline-block', textDecoration: 'none' }} target="_blank" rel="noreferrer">
              {content.customPackageButtonLabel}
            </a>
          </div>
        </div>
      )}
