...
type ToolPreviewProps = {
  content?: typeof defaultMainWebsiteContent.home.previewSection
}

export default function ToolPreview({ content = defaultMainWebsiteContent.home.previewSection }: ToolPreviewProps) {
  return (
    <>
      <style>{`
              ...
              <AnimateOnScroll key={tool.name} direction={idx % 2 === 0 ? 'left' : 'right'}>
                <div className={`preview-block ${idx % 2 !== 0 ? 'reverse' : ''}`}>
                  <div className="preview-text-side">
                    ...
                    <Link href={tool.ctaHref} className="preview-cta">{tool.ctaLabel}</Link>
                  </div>
                    ...
                    {tool.isPhoto ? (
                      <div className="photo-mockup">
                        <div className="photo-before-after">
                          <div className="photo-box">
                            ...
                            <div className="photo-box-label">{tool.beforeLabel}</div>
                          </div>
                          <div className="photo-arrow">→</div>
                          <div className="photo-box">
                            <div className="photo-box-icon">📸</div>
                            <div className="photo-box-label">{tool.afterLabel}</div>
                          </div>
                        </div>
                        <div className="photo-result">
                          <div className="photo-result-label">{tool.posesLabel}</div>
                      ...
                      <div className="mockup-content">
                        <div className="mockup-title">
                          <span>{tool.icon}</span>
                          <span>{tool.mockupTitle}</span>
                        </div>
                        {tool.mockupRows?.map(row => (
                          <div key={`${tool.name}-${row.label}`} className="mockup-row">
