...
const humanizeCategoryFallback = (value: string) =>
  value
    .trim()
    .replace(/^(cat|category)-/i, '')
    .split(/[-_]+/)
    .filter(Boolean)
...
export function LabourMastersPanel() {
  ...
  const getLinkedStateLabel = (option: LabourMasterOption) => {
    const linkedStateId = option.stateOptionId || ''
    if (!linkedStateId) return 'Not linked yet'
    const linkedState = (optionsByKey.state || []).find(item => item.id === linkedStateId)
    if (!linkedState) return 'Missing state'
    return linkedState.isActive ? linkedState.label : `${linkedState.label} (Inactive)`
  }
  ...
  const stateOptionsForEditor = useMemo(
    () =>
      (optionsByKey.state || [])
        .filter(option => option.isActive || option.id === optionDraft.stateOptionId)
        .sort((left, right) => left.label.localeCompare(right.label, 'en', { sensitivity: 'base' })),
    [optionDraft.stateOptionId, optionsByKey.state]
  )
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setCategories(
        Array.isArray(labourPayload.adminCategories)
          ? labourPayload.adminCategories
          : Array.isArray(labourPayload.categories)
            ? labourPayload.categories
            : []
      )
    } catch (fetchError) {
    ...
    () =>
      (snapshot?.categoryDependencies || [])
        ...
        .map<MappedCategoryRow>(dependency => ({
          ...
          categoryLabel: categoryMap.get(dependency.categoryId) || humanizeCategoryFallback(dependency.categoryId),
          isCategoryAvailable: categoryMap.has(dependency.categoryId),
          isActive: dependency.isActive
        }))
  ...
  const saveDependency = async () => {
    ...
    try {
      const sanitizedDependencyDraft = {
        ...dependencyDraft,
        categoryIds: dependencyDraft.categoryIds.filter(categoryId => availableDependencyCategoryIds.has(categoryId))
      }
      const response = await fetch('/api/admin/labour/masters', {
        ...
        body: JSON.stringify(
          editingDependencyId
            ? { kind: 'dependency', id: editingDependencyId, payload: sanitizedDependencyDraft }
            : { kind: 'dependency', payload: sanitizedDependencyDraft }
        )
      })
  ...
  const selectedMasterOptions = useMemo(() => {
    ...
    const searched = baseOptions.filter(option => {
      ...
      const searchableValues = [option.label, option.value, option.description]
      if (selectedMasterKey === 'city') {
        searchableValues.push(getLinkedStateLabel(option))
      }
  ...
  const availableDependencyCategoryIds = useMemo(
    () => new Set(categories.map(category => category.id)),
    [categories]
  )

  const orphanedDependencyCategoryLabels = useMemo(
    () =>
      dependencyDraft.categoryIds
        .filter(categoryId => !availableDependencyCategoryIds.has(categoryId))
        .map(categoryId => {
          const mappedRow = selectedIndustryCategoryRows.find(row => row.categoryId === categoryId)
          return mappedRow?.categoryLabel || humanizeCategoryFallback(categoryId)
        }),
    [availableDependencyCategoryIds, dependencyDraft.categoryIds, selectedIndustryCategoryRows]
  )
    ...
    <section className="surface-card content-panel">
      ...
      <div style={{ overflowX: 'auto' }}>
        {selectedGroup === 'categoryDependencies' ? (
                    ...
                    <tbody>
                      {selectedIndustryCategoryRows.map(row => (
                        <tr key={row.id}>
                          ...
                          <td style={{ padding: '12px 10px 12px 0', borderTop: cardBorder, color: '#13233f', fontSize: 14, fontWeight: 600 }}>
                            {row.categoryLabel}
                            {!row.isCategoryAvailable ? (
                              <span className="chip warning" style={{ marginLeft: 8 }}>Deleted / unavailable</span>
                            ) : null}
                          </td>
                          <td style={{ padding: '12px 10px 12px 0', borderTop: cardBorder }}>
                            <span className={`chip ${row.isActive ? 'success' : 'warning'}`}>{row.isActive ? 'Active' : 'Inactive'}</span>
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, minWidth: selectedMasterKey === 'city' ? 980 : 860 }}>
              <thead>
                <tr style={{ textAlign: 'left' }}>
                  {(
                    selectedMasterKey === 'city'
                      ? ['Label', 'Value', 'Linked State', 'Description', 'Sort', 'Status', 'Actions']
                      : ['Label', 'Value', 'Description', 'Sort', 'Status', 'Actions']
                  ).map(label => (
                    <th key={label} style={{ padding: '0 0 10px', fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#5e789d' }}>{label}</th>
                  ))}
                </tr>
        ...
        <div style={modalOverlayStyle}>
          <div style={modalCardStyle}>
              ...
              <div>
                ...
                <p className="panel-copy">
                  {optionDraft.masterKey === 'business_type'
                    ...
                    : optionDraft.masterKey === 'city'
                      ? 'Create a city master and optionally link it to the State that should own it for future dependent dropdowns.'
                    : editingOptionId
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              {optionDraft.masterKey === 'city' ? (
                <div className="field" style={fieldStyle}>
                  <label>Linked State</label>
                  <select
                    value={optionDraft.stateOptionId}
                    onChange={event => setOptionDraft(current => ({ ...current, stateOptionId: event.target.value }))}
                  >
                    <option value="">No linked state yet</option>
                    {stateOptionsForEditor.map(option => (
              ...
              <div className="field" style={{ ...fieldStyle, gridColumn: '1 / -1' }}>
                ...
                <div style={checkboxPanelStyle}>
                  ...
                  {!dependencyHasBusinessTypeSelection ? (
                    ...
                    <>
                      {orphanedDependencyCategoryLabels.length > 0 ? (
                        <div style={{ border: '1px solid #f59e0b', borderRadius: 12, padding: '10px 12px', background: '#fff7ed', color: '#9a3412', fontSize: 12, lineHeight: 1.6 }}>
                          Deleted or unavailable category mappings are no longer selectable: {orphanedDependencyCategoryLabels.join(', ')}. Saving this mapping set will remove those orphaned entries automatically.
                        </div>
                      ) : null}
