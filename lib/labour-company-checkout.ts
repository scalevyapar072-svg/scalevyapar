...
export type CheckoutSummary = {
  ...
  gstAmount: number
  taxLabel: string
  taxType: 'intra_state' | 'inter_state' | 'gst_disabled'
  total: number
  creditsLabel: string
...
export const createCheckoutSummary = ({
  ...
  buyerStateOverride?: string | null
}): CheckoutSummary => {
  ...
  const taxSettings = resolveLabourCompanyTaxSettings(pricingPage.checkout.taxSettings, pricingPage.checkout.gstPercentage)
  const taxResolution = resolveCompanyTaxResolution({
    buyerGstin: gstinOverride || undefined,
    buyerState: buyerStateOverride || undefined,
    sellerGstin: taxSettings.sellerGstin,
    sellerState: taxSettings.sellerState,
    sellerStateCode: taxSettings.sellerStateCode,
    settings: taxSettings
  })
  const taxBreakdown = calculateExclusiveTaxBreakdown(subtotal, taxResolution)
  const gstPercent = taxBreakdown.totalGstPercent || sanitizeNumber(pricingPage.checkout.gstPercentage) || DEFAULT_GST_PERCENT
  const gstAmount = taxBreakdown.taxAmount
  const total = taxBreakdown.totalAmount
  const hasBuyerTaxLocation = Boolean((gstinOverride || '').trim() || (buyerStateOverride || '').trim())
  ...
  return {
    ...
    gstPercent,
    gstAmount,
    taxLabel: hasBuyerTaxLocation ? (taxBreakdown.label || `GST (${gstPercent}%)`) : `GST (${gstPercent}%)`,
