import {
  ...
  seededBusinessTypeCatalog,
  seededIndustryCategoryCatalog,
  seededIndustryDependencyBlueprints
...
export interface LabourStateCityGroup {
  stateOptionId: string
  stateLabel: string
  cities: LabourMasterOption[]
}
...
export interface LabourMastersSnapshot {
  ...
  activeStates: LabourMasterOption[]
  activeCities: LabourMasterOption[]
  activeCitiesByState: LabourStateCityGroup[]
  storage: 'supabase' | 'json'
}

export interface LabourSelectableCityOptions {
  activeStates: LabourMasterOption[]
  activeCitiesWithActiveState: LabourMasterOption[]
  activeCitiesByState: LabourStateCityGroup[]
}
...
export const labourMasterDefinitions: Record<LabourMasterKey, LabourMasterDefinition> = {
  ...
  worker_salary_type: {
    key: 'worker_salary_type',
    label: 'Worker Salary Type',
    group: 'worker',
    description: 'Salary-type options used in labour worker registration and profile flows.'
  },
  company_status: {
    key: 'company_status',
    label: 'Status',
...
const buildMasterReferenceCandidates = (masterKey: LabourMasterKey, value: string) => {
  ...
  const legacyPrefix = compareText(`master-${masterKey}-`)

  if (normalizedValue.startsWith(legacyPrefix)) {
    const suffix = normalizedValue.slice(legacyPrefix.length).trim()
    if (suffix) {
      candidates.add(suffix)
      candidates.add(suffix.replace(/[-_]+/g, ' '))
      candidates.add(suffix.replace(/-/g, '_'))
    }
  }
...
export const buildSelectableLabourCityOptions = (options: LabourMasterOption[]): LabourSelectableCityOptions => {
  const stateOptions = sortLabourMasterOptions(options.filter(option => option.masterKey === 'state'))
  const cityOptions = sortLabourMasterOptions(options.filter(option => option.masterKey === 'city'))

  const statesById = new Map(stateOptions.map(option => [option.id, option]))

  const isStateActive = (option: LabourMasterOption) => option.isActive !== false
  const hasSelectableActiveState = (option: LabourMasterOption) => {
    const linkedStateId = option.stateOptionId?.trim() || ''
    if (!linkedStateId) return false
    const linkedState = statesById.get(linkedStateId)
    return Boolean(linkedState && linkedState.isActive !== false)
  }
  const isSelectableCity = (option: LabourMasterOption) => {
    if (option.isActive === false) return false
    return hasSelectableActiveState(option)
  }

  const activeStates = stateOptions.filter(isStateActive)
  const activeCitiesWithActiveState = cityOptions.filter(isSelectableCity)

  const activeCitiesByState = activeStates.map<LabourStateCityGroup>(stateOption => ({
    stateOptionId: stateOption.id,
    stateLabel: stateOption.label,
    cities: activeCitiesWithActiveState.filter(cityOption => (cityOption.stateOptionId || '') === stateOption.id)
  })).filter(group => group.cities.length > 0)

  return {
    activeStates,
    activeCitiesWithActiveState,
    activeCitiesByState
  }
}

export const buildLabourLocationOptions = (options: LabourMasterOption[]) => {
  const selectableCityOptions = buildSelectableLabourCityOptions(options)
  return {
    activeStates: selectableCityOptions.activeStates,
    activeCities: selectableCityOptions.activeCitiesWithActiveState,
    activeCitiesByState: selectableCityOptions.activeCitiesByState
  }
}
...
const optionMatchesMasterValue = (option: LabourMasterOption, value: string) => {
  const candidates = buildMasterReferenceCandidates(option.masterKey, value)
  ...
  const optionValues = [
    ...
    option.value,
    option.label,
    ...getSeededMasterAliases(option.masterKey, option.value)
  ]

  return candidates.some(candidate =>
    optionValues.some(optionValue => {
      const normalizedOptionValue = compareText(optionValue)
      if (!normalizedOptionValue) return false
      return normalizedOptionValue === candidate || slugifyLabourMaster(optionValue) === slugifyLabourMaster(candidate)
    })
  )
}
...
export const filterBusinessTypesByIndustryDependency = (
  ...
  useSeededIndustryFallback = false
) => {
  ...
  const matchingDependencies = industryBusinessDependencies.filter(dependency => {
    if (!dependency.isActive) return false

    return matchingIndustryCategoryIds.includes(dependency.industryCategoryOptionId) ||
      visibleIndustryCategoryOptions.some(
        option =>
          matchingIndustryCategoryIds.includes(option.id) &&
          optionMatchesMasterValue(option, dependency.industryCategoryOptionId)
      )
  })

  const dependencyMatchedOptions = visibleBusinessTypeOptions.filter(option =>
    matchingDependencies.some(dependency =>
      dependency.businessTypeOptionId === option.id || optionMatchesMasterValue(option, dependency.businessTypeOptionId)
    )
  )

  if (!useSeededIndustryFallback) {
    return dependencyMatchedOptions
  }

  if (dependencyMatchedOptions.length > 0) {
    return dependencyMatchedOptions
  }

  const matchingIndustryValues = new Set(
    visibleIndustryCategoryOptions
      .filter(option => matchingIndustryCategoryIds.includes(option.id))
      .flatMap(option => [option.label, option.value, option.slug])
      .map(compareText)
      .filter(Boolean)
  )
  matchingIndustryValues.add(compareText(selectedIndustryCategory))

  const seededBusinessTypeLabels = seededIndustryDependencyBlueprints
    .filter(blueprint =>
      matchingIndustryValues.has(compareText(blueprint.industryLabel)) ||
      matchingIndustryValues.has(compareText(blueprint.industryValue))
    )
    .flatMap(blueprint => blueprint.businessTypeLabels)

  if (seededBusinessTypeLabels.length === 0) {
    return dependencyMatchedOptions
  }

  const seededFallbackOptions = visibleBusinessTypeOptions.filter(option =>
    seededBusinessTypeLabels.some(label => optionMatchesMasterValue(option, label))
  )

  const seenOptionIds = new Set<string>()
  return [...dependencyMatchedOptions, ...seededFallbackOptions].filter(option => {
...
export const filterCategoriesByLabourDependency = <T extends { id: string; isActive?: boolean }>(
  categories: T[],
  categoryDependencies: LabourCategoryDependency[],
  optionsByKey: Partial<Record<LabourMasterKey, LabourMasterOption[]>>,
