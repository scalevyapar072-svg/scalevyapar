...
const optionMatchesSeed = (
  ...
  const seedSlug = slugifyLabourMaster(seed.value || seed.label)
  const canonicalSeedId = `master-${masterKey}-${seedSlug || 'option'}`
  const comparisons = [
    seed.label,
    seed.value,
    seedSlug,
  ...
  seededCatalog.forEach((seed, index) => {
    ...
    if (existing) {
      ...
      existing.description = masterKey === 'business_type'
        ...
        : existing.description
      // Preserve any admin-managed active/hidden state for existing master rows.
      // This sync should add missing seeded options and refresh labels/values,
      // but it should not silently re-activate options an admin intentionally hid.
...
const collapseDuplicateOptions = (
  ...
  const chooseCanonicalOption = (current: LabourMasterOption, candidate: LabourMasterOption) => {
    if (current.masterKey === 'city' && candidate.masterKey === 'city') {
      const currentStateOptionId = current.stateOptionId?.trim() || ''
      const candidateStateOptionId = candidate.stateOptionId?.trim() || ''
      if (candidateStateOptionId && !currentStateOptionId) return candidate
      if (currentStateOptionId && !candidateStateOptionId) return current
    }
  ...
  options.forEach(option => {
    ...
    if (!canonical.stateOptionId && duplicate.stateOptionId) {
      canonical.stateOptionId = duplicate.stateOptionId
    }
  ...
  return {
    options: Array.from(canonicalBySemanticKey.values()).map(option => ({
      ...option,
      stateOptionId: option.stateOptionId ? resolvedOptionId(option.stateOptionId) : ''
    })),
...
const normalizeOptionRecord = (value: unknown, fallback: LabourMasterOption): LabourMasterOption => {
  ...
  return {
    ...
    stateOptionId: normalizeString(candidate.stateOptionId, fallback.stateOptionId || ''),
...
const createSnapshot = (data: LabourMastersData, storage: 'supabase' | 'json'): LabourMastersSnapshot => {
  const options = sortLabourMasterOptions(data.options)
  const locationOptions = buildLabourLocationOptions(options)
  ...
  return {
    ...
    activeStates: locationOptions.activeStates,
    activeCities: locationOptions.activeCities,
    activeCitiesByState: locationOptions.activeCitiesByState,
    storage
  }
}
...
const normalizeOptionInput = (
  ...
  const timestamp = new Date().toISOString()
  const rawStateOptionId = normalizeString(payload.stateOptionId, existing?.stateOptionId || '')
  let stateOptionId = ''

  if (masterKey === 'city' && rawStateOptionId) {
    const linkedState = findMatchingMasterOption(
      options.filter(option => option.masterKey === 'state'),
      rawStateOptionId
    )
    if (!linkedState) {
      throw new Error('Linked State was not found in master values. Please refresh and try again.')
    }
    stateOptionId = linkedState.id
  }
...
export const createLabourMasterOption = async (
  ...
  const record = normalizeOptionInput(payload, data.options)
...
export const updateLabourMasterOption = async (
  ...
  const record = normalizeOptionInput(payload, data.options, existing)
...
export const purgeInactiveLabourCategoryDependenciesForCategory = async (
  categoryId: string,
  actor: string
): Promise<LabourMastersSnapshot> => {
  const { data, storage: existingStorage } = await readDataWithStorage()
  const normalizedCategoryId = normalizeString(categoryId)

  if (!normalizedCategoryId) {
    throw new Error('Labour Category is required to clear inactive mappings.')
  }

  const removedRecords = data.categoryDependencies.filter(
    record => record.categoryId === normalizedCategoryId && !record.isActive
  )

  if (removedRecords.length === 0) {
    return createSnapshot(data, existingStorage)
  }
  ...
  if (storage === 'supabase') {
    await writeAuditLog(
      ...
      `Removed ${removedRecords.length} inactive Labour Category mapping${removedRecords.length === 1 ? '' : 's'} for category ${normalizedCategoryId}`,
      actor
    )
  }
